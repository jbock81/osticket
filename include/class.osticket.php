...
class osTicket {
    ...
    function get_path_info() {
        ...
        if(isset($server['PATH_INFO']) && !empty($server['PATH_INFO'])){
			$url= $server['PATH_INFO'];
		}

        if(isset($server['ORIG_PATH_INFO']) && !empty($server['ORIG_PATH_INFO'])){
			$url= $server['ORIG_PATH_INFO'];
		}
		if(isset($server['REQUEST_URI']) && !empty($server['REQUEST_URI'])){
			$url= str_replace($server['DOCUMENT_URI'],'',$server['REQUEST_URI']);
		}
